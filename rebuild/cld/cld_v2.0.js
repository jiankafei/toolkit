...
/*
用法：
var $cld=$(document.body).cld(selector);//$cld表示日期选择器控件
*注：
所有的日期都是*_f和*_s这两个类
*/
(function ($) {
	$.fn.cld = function (selector) {
		/*selector*/
		var $objs = $(selector);
		var yyNow = 0; //当前年
		var now = 0; //当前月
		...
		$objs.val(function () {
			if (!$(this).attr('class').match(reg) && $(this).val() == '') {
				var $date = new Date();
		...
		$objs.on('focus', function () {
			...
			if (cls.match(/_f\b/)) {
				who = 'f';
				var $sideVal = $('.' + (cls.match(/\b\w+_f\b/))[0].replace(/_f/, '_s')).val();
				if ($sideVal) {
					o = $sideVal.split('-');
				} else {
					o = null;
				...
				who = 's';
				var $sideVal = $('.' + (cls.match(/\b\w+_s\b/))[0].replace(/_s/, '_f')).val();
				if ($sideVal) {
					o = $sideVal.split('-');
				} else {
					o = null;
		...
		var $footer = $('<footer><p>说明:</p></footer>');
		$cld.append($footer);
		/*操作1,切换日期操作*/
