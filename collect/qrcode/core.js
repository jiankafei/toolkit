 ...
 * 由 jquery-qrcode 修改而来，添加了对中文字符的支持，使用 ES6 重写
 */
...
const QRUtil = {
  ...
  getLostPoint(qrCodeModel) {
    let moduleCount = qrCodeModel.getModuleCount();
    let lostPoint = 0;
    ...
    for (let row = 0; row < moduleCount; row++) {
      for (let col = 0; col < moduleCount; col++) {
        let sameCount = 0;
        let dark = qrCodeModel.isDark(row, col);
        for (let r = -1; r <= 1; r++) {
    ...
    for (let row = 0; row < moduleCount - 1; row++) {
      for (let col = 0; col < moduleCount - 1; col++) {
        let count = 0;
        if (qrCodeModel.isDark(row, col)) count++;
        if (qrCodeModel.isDark(row + 1, col)) count++;
        if (qrCodeModel.isDark(row, col + 1)) count++;
        if (qrCodeModel.isDark(row + 1, col + 1)) count++;
        if (count == 0 || count == 4) {
          lostPoint += 3;
        }
      }
    ...
    for (let row = 0; row < moduleCount; row++) {
      for (let col = 0; col < moduleCount - 6; col++) {
        if (
          qrCodeModel.isDark(row, col) &&
          !qrCodeModel.isDark(row, col + 1) &&
          qrCodeModel.isDark(row, col + 2) &&
          qrCodeModel.isDark(row, col + 3) &&
          qrCodeModel.isDark(row, col + 4) &&
          !qrCodeModel.isDark(row, col + 5) &&
          qrCodeModel.isDark(row, col + 6)
        ) {
          lostPoint += 40;
        }
      }
    ...
    for (let col = 0; col < moduleCount; col++) {
      for (let row = 0; row < moduleCount - 6; row++) {
        if (
          qrCodeModel.isDark(row, col) &&
          !qrCodeModel.isDark(row + 1, col) &&
          qrCodeModel.isDark(row + 2, col) &&
          qrCodeModel.isDark(row + 3, col) &&
          qrCodeModel.isDark(row + 4, col) &&
          !qrCodeModel.isDark(row + 5, col) &&
          qrCodeModel.isDark(row + 6, col)
        ) {
          lostPoint += 40;
        }
      }
    ...
    for (let col = 0; col < moduleCount; col++) {
      for (let row = 0; row < moduleCount; row++) {
        if (qrCodeModel.isDark(row, col)) darkCount++;
      }
    }
...
class QRCodeModel {
  ...
  addData(data) {
    const newData = new QR8bitByte(utf16to8(data));
...
module.exports = { QRCodeModel, QRErrorCorrectLevel };
